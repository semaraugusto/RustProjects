...
fn run_instruction(paper: &[Vec<bool>], instr: &Instruction) -> Vec<Vec<bool>> {
    // println!("INSTRUCTION {:?}", instr);
    let mut output: Vec<Vec<bool>>;
...
fn main() {
    ...
    let result = paper.iter().fold(0, |outer_acc, row| {
        outer_acc + row.iter().fold(0, |inner_acc, val| inner_acc + *val as u32)
    });
...
fn print_matrix(mat: &Vec<Vec<bool>>, instr: Option<&Instruction>) {
    println!("instruction {:?}", instr);
